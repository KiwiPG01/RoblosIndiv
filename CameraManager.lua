	...
	local dataTable = setmetatable(
		{
			...
			--// Configurations //--
			VerticalAngleLimits = NumberRange.new(-89, 89),
			----
...
function CLASS:SetMouseStep(steppedIn)
	...
	if (steppedIn == true) and (LOCAL_PLAYER.Character:FindFirstChildWhichIsA("Tool")) then
		print(LOCAL_PLAYER.Character:FindFirstChildWhichIsA("Tool"))
...
USER_INPUT_SERVICE.InputBegan:Connect(function(inputObject, gameProcessedEvent)
	if (gameProcessedEvent == false) and (singleton.IsEnabled == true) then
		if (inputObject.KeyCode == Enum.KeyCode.Z) then
			if singleton.ShoulderDirection == 1 then
				singleton:SetShoulderDirection(-1)
			elseif singleton.ShoulderDirection == -1 then
				singleton:SetShoulderDirection(1)
			end
		end
